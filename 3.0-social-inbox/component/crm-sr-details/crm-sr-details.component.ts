...
@Component({
    selector: 'app-crm-sr-details',
    templateUrl: './crm-sr-details.component.html',
    styleUrls: ['./crm-sr-details.component.scss'],
    standalone: false
})
export class CrmSrDetailsComponent implements OnInit {
  constructor(
  ...
  ngOnInit(): void {
    ...
    const crmdetailsobject = {
      ...
      RequestTypes: BTCCrmRequestType.ServiceRequest,
    };
    this.isLoading = true;
  ...
  convertStringToJson(data){
    try {
      ...
      if (error instanceof SyntaxError) {
        return []
        // Handle the error or fix the JSON data
      } else {
        return []
      }
    }
  ...
  imageZoom(attachmentsData){
    const attachments = [];
    JSON.parse(attachmentsData).forEach((res)=>{
      attachments.push({
        mediaType: 2,
        mediaUrl: 'data:image/png;base64,'+res.VersionData
      });
    })
