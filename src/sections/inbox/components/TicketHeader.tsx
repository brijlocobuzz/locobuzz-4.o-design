...
export function TicketHeader({
  ...
  const handleToggleInsightOption = (option: string) => {
    if (messageInsights && onMessageInsightsChange) {
      // Handle nested paths like "essentials.customerName"
                ...
                <div className="relative">
                  ...
                  {showInsightsDropdown && (
                    <>
                      ...
                      />
                      <div className="absolute left-0 top-full z-50 mt-2 w-64 rounded-lg border border-slate-200 bg-white py-2 shadow-lg dark:border-slate-700 dark:bg-slate-800">
                        ...
                        <button
                          onClick={() => handleToggleInsightOption('reasonSurfaced')}
                          className="flex w-full items-center justify-between px-4 py-2 text-left text-sm hover:bg-slate-50 dark:hover:bg-slate-700"
                        >
                          <span className="text-slate-700 dark:text-slate-300">Reason Surfaced</span>
                          {messageInsights.selectedInsights.reasonSurfaced && (
                            <Check className="h-4 w-4 text-indigo-600 dark:text-indigo-400" />
                          )}
                        </button>
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs text-slate-500 dark:text-slate-400">
            ...
            <span className="font-medium text-green-600 dark:text-green-400">
              ...
              {ticket.sla.resolution.remainingMinutes % 60}m remaining
            </span>
          </span>
        </div>
      </div>
