...
type SocialAction = 'dm-link' | 'send-dm' | 'email' | 'feedback-link'
...
export function ReplyComposer({
  ...
  // Social action: send feedback form
  ...
  const isSocialChannel = isTwitter || isFacebook || isInstagram
  ...
  const handleSocialAction = (action: SocialAction) => {
    ...
    console.log(`Social action: ${action}`, { platform: ticketPlatform, sendFeedbackForm })
    // In real implementation, this would:
    // - 'dm-link': Insert a DM link message template
    // - 'send-dm': Switch to DM conversation mode
    // - 'email': Request customer email via DM/message
    // - 'feedback-link': Include feedback form link in message
  }
      ...
      <div className="p-4">
        ...
        <div className="mt-3 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            {/* Social Channel Quick Actions - Twitter, Facebook, Instagram in reply mode */}
            {isSocialChannel && replyMode === 'reply' && (
              <>
                <div className="mx-2 h-6 w-px bg-slate-200 dark:bg-slate-700" />
                {/* DM Link - Twitter & Facebook only */}
                {(isTwitter || isFacebook) && (
                  <button
                    onClick={() => handleSocialAction('dm-link')}
                    title="Send DM link in reply"
                ...
                {/* Send as DM - All social channels */}
                <button
                  onClick={() => handleSocialAction('send-dm')}
                  title="Switch to DM conversation"
                  className="flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-xs font-medium text-indigo-600 hover:bg-indigo-50 dark:text-indigo-400 dark:hover:bg-indigo-950/30"
                >
                ...
                <div className="mx-2 h-6 w-px bg-slate-200 dark:bg-slate-700" />
                <label className="flex cursor-pointer items-center gap-2 rounded-lg px-3 py-1.5 text-xs font-medium text-purple-600 hover:bg-purple-50 dark:text-purple-400 dark:hover:bg-purple-950/30">
                  <input
                    type="checkbox"
                    checked={sendFeedbackForm}
                    onChange={(e) => setSendFeedbackForm(e.target.checked)}
                    className="h-3.5 w-3.5 rounded border-purple-300 text-purple-600 focus:ring-purple-500 dark:border-purple-700 dark:bg-slate-800"
                  />
